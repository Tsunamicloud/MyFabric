...
import com.tsunamicloud.tsunami.block.entity.SaualpiteBlasterBlockEntity;
...
public class SaualpiteBlasterBlock extends BlockWithEntity implements BlockEntityProvider {
    public static final DirectionProperty FACING = Properties.HORIZONTAL_FACING;

    public SaualpiteBlasterBlock(Settings settings) {
        super(settings);
    }
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new SaualpiteBlasterBlockEntity(pos, state);
    }
    ...
    //破坏该方块实体时，内容物以掉落物形式掉落出来
    @Override
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof SaualpiteBlasterBlockEntity) {
                ItemScatterer.spawn(world, pos, (SaualpiteBlasterBlockEntity)blockEntity);
    ...
    @Nullable
    @Override
    //tick方法在SaualpiteBlasterBlockEntity中写出
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlockEntities.SAUALPITE_BLASTER, SaualpiteBlasterBlockEntity::tick);
    }
